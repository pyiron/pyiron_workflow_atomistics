...
def test_supercell_targets_half_n_atoms():
    s = create_coexistence_supercell("Al", "fcc", a=4.05, n_atoms=8000)
    assert len(s) == 4000  # fcc cubic = 4 atoms; 4*10^3 = 4000 ~ 8000/2


def test_freeze_half_fixes_lower_half():
    s = create_coexistence_supercell("Al", "fcc", a=4.05, n_atoms=2000)
    frozen = freeze_half(s, axis=2, fraction=0.5)
...
def test_unfreeze_clears_constraints():
    s = create_coexistence_supercell("Al", "fcc", a=4.05, n_atoms=2000)
    assert len(unfreeze(freeze_half(s)).constraints) == 0


def test_strain_scales_only_c():
    s = create_coexistence_supercell("Al", "fcc", a=4.05, n_atoms=2000)
    c0 = s.cell[2, 2]
    strained = strain_cell_along_z(s, 1.05)
