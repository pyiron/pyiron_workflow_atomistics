...
def parse_inputs():
    ...
    if potential == 'Zope':
        ...
        c = 4.6927
        Ecoh = -4.8722
    return args.tilt, args.potential, args.formula, args.xmax, args.ymax, args.zmax, a, c, Ecoh


def loop_replicate(formula, a, c, dirs, xmax, ymax, zmax):
    ''' Replicate CSL [0001] slabs. '''
    s0 = HexagonalClosedPacked(symbol=formula,
                               latticeconstant=(a,c),
                               directions=dirs,
                               size=[1, 1, 10])
    ...
    v = dirs[2]
    dspace = 1 / np.sqrt(4/3 * (v[0]**2 + v[0]*v[1] + v[1]**2) / a**2 + v[3]**2 / c**2)
...
def slice_cell(infile, xmax=10, ymax=5, zmax=30, disp=[0.0, 0.0, 0.0]):
    ...
    # Square up the cell
    aa, bb, cc = s.cell
    assert (abs(aa[1]) + abs(aa[2]) + abs(bb[0]) + abs(bb[2]) < 1e-5), 'Cell not very orthogonal!'
    ...
    npl = sum((s.positions[:, 2] < zmid + Z_THRESH) & (s.positions[:, 2] > zmid - Z_THRESH))
    ...
    if nmin != npl:
        ...
        mask2 = (s.positions[:, 2] < zmid + Z_THRESH) & (s.positions[:, 2] > zmid - Z_THRESH)
...
def write_gb(filename, top_crystal, bottom_crystal, gap, vac, gb_normal=2):
    ...
    write_vasp(f'{filename}.POSCAR', tc, label=f'bicrystal_gap{gap}_normal{gb_normal}', direct=True, sort=True)
...
if __name__ == '__main__':
    ...
    print(f'Generating {formula} {tilt} STGB structures for the {potential} potential...')

    # Gather HCP plane data
    df = pd.read_csv(os.path.join('data', f"GB_{tilt.strip('-')}_sym_tilt.txt"), sep=' ')
    ...
    if tilt == '0001':
        for i, row in df.iterrows():
            ...
            upper_dirs = [row['uxu':'uxw'].tolist(), (-row['uzu':'uzw']).tolist(), row['uyu':'uyw'].tolist()]
            upper, _ = loop_replicate(formula, a, c, upper_dirs, xmax, ymax, zmax0)
            upper.positions[:, 2] += P_SHIFT
            write_vasp(os.path.join(root_folder, 'POSCAR_SUBSTRATE_2'), upper, label=f'{formula} upper {upper_dirs}', 
                       direct=True, sort=True)

            lower_dirs = [row['lxu':'lxw'].tolist(), (-row['lzu':'lzw']).tolist(), row['lyu':'lyw'].tolist()]
            lower, ref = loop_replicate(formula, a, c, lower_dirs, xmax, ymax, zmax0)
            write_vasp(os.path.join(root_folder, 'POSCAR_SUBSTRATE_1'), lower, label=f'{formula} lower {lower_dirs}', 
                       direct=True, sort=True)

            write_vasp(os.path.join(root_folder, 'POSCAR_SUBSTRATE_REF'), ref, label=f'{formula} lower {lower_dirs}', 
                       direct=True, sort=True)
            write_gb(os.path.join(root_folder, 'bicrystal'), upper, lower, gap=0.0, vac=10)
