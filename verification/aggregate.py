...
EXP_REF = {
    ...
    "Al": dict(C11=107.3, C12=60.9, C44=28.3, K_VRH=76.4, G_VRH=26.1, E=70.4, nu=0.346,
               source="Simmons & Wang 1971 (Vallin 1964 / Kamm & Alers 1964)"),
    "Cu": dict(C11=168.4, C12=121.4, C44=75.4, K_VRH=137.1, G_VRH=47.3, E=127.4, nu=0.345,
               source="Simmons & Wang 1971; Ledbetter & Naimon, JPCRD 3, 897 (1974)"),
    "Si": dict(C11=165.6, C12=63.9, C44=79.5, K_VRH=97.8, G_VRH=66.5, E=162.6, nu=0.223,
               source="Hall, Phys. Rev. 161, 756 (1967); McSkimin & Andreatch 1964"),
    "Fe": dict(C11=233.0, C12=135.0, C44=117.5, K_VRH=167.7, G_VRH=82.6, E=212.7, nu=0.288,
               source="Rayne & Chandrasekhar, Phys. Rev. 122, 1714 (1961); Ledbetter & Reed 1973"),
    "Ni": dict(C11=254.0, C12=155.0, C44=123.0, K_VRH=188.0, G_VRH=85.0, E=222.0, nu=0.30,
               source="Ledbetter & Reed, JPCRD 2, 531 (1973); Neighbours et al. 1952"),
    "W": dict(C11=523.3, C12=204.5, C44=160.7, K_VRH=310.8, G_VRH=160.2, E=410.1, nu=0.28,
...
def build_rows(recs):
    rows = []
    ordered = [m for m in ORDER if m in recs] + sorted(m for m in recs if m not in ORDER)
    for mat in ordered:
        r = recs[mat]
        g = r.get("grace") or {}
        mp = r.get("mp") or {}
        ...
        row = {
            "material": mat, "mp_id": r.get("mp_id"),
            "grace_ok": r.get("grace_ok"), "grace_stable": g.get("mechanically_stable"),
            "mp_state": mp.get("state"), "mp_reliable": mp.get("reliable"),
            "grace_K": g.get("K_VRH"), "mp_K": mp.get("K_VRH"), "exp_K": exp.get("K_VRH"),
            "grace_G": g.get("G_VRH"), "mp_G": mp.get("G_VRH"), "exp_G": exp.get("G_VRH"),
            "grace_E": g.get("youngs_modulus"), "mp_E": mp.get("youngs_modulus"),
            "grace_nu": g.get("poisson_ratio"), "mp_nu": mp.get("poisson_ratio"),
            "grace_A_U": g.get("universal_anisotropy"), "mp_A_U": mp.get("universal_anisotropy"),
            "grace_C11": (gC[0, 0] if gC is not None else None), "mp_C11": mp.get("C11"), "exp_C11": exp.get("C11"),
            "grace_C12": (gC[0, 1] if gC is not None else None), "mp_C12": mp.get("C12"), "exp_C12": exp.get("C12"),
            "grace_C44": (gC[3, 3] if gC is not None else None), "mp_C44": mp.get("C44"), "exp_C44": exp.get("C44"),
        }
...
def main():
    ...
    L.append("| Mat | mp-id | MP state | GRACE K | MP K | EXP K | GRACE G | MP G | EXP G | ref | K err% | G err% | GRACE stable |")
    L.append("|-----|-------|----------|--------:|-----:|------:|--------:|-----:|------:|:---:|-------:|-------:|:------------:|")
    for r in rows:
        L.append(
    ...
    L.append("| Mat | GRACE C11 | MP C11 | EXP C11 | GRACE C12 | MP C12 | EXP C12 | GRACE C44 | MP C44 | EXP C44 |")
    L.append("|-----|----------:|-------:|--------:|----------:|-------:|--------:|----------:|-------:|--------:|")
    for r in rows:
        L.append(
