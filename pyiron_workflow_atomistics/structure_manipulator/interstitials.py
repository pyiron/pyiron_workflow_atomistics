...
def filter_condition(sys, pos, rvv, distance_min, distance_max, axis, rvv_min, rvv_max):
    ...
    if not within_distance:
        within_distance = (sys.boxdims[axis]-width <= np.round(pos[axis], decimals=3)  < np.round(sys.boxdims[axis], decimals=3))
...
def tabulate_voids(void_ratios, void_count):
    ...
    y = ax.table(cellText=np.array([void_ratios, void_count]).T,colLabels=collabel,loc='center')
    y.set_fontsize(14)
    y.scale(2,2)
    
def calculate_voids(inputfile, format, alat, pf, extra_rvv=0.5, tabulate=True, write=True):
    """
    ...
    for count, pos in enumerate(sys.atoms.positions):
        if sys.atoms.types[count] == maxcomp:
            indices = np.array([sys.atoms.types[x] for x in sys.atoms.neighbors.index[count]])
            #print(len(indices))
    ...
    sys.write.file(f'intermediate_{inputfile}', customkeys=['rvv'], customvals=np.array([rlist]).T)
...
def filter_and_cluster_atoms(sys,
                            distance,
                            axis,
                            rvv,
                            write=True):
    ...
    for count, pos in enumerate(sys.atoms.positions):
        if sys.atoms.types[count] == maxcomp:
            condition = filter_condition(sys, pos, rlist[count],
                                            d_min, d_max,
                                            axis,
                                            rvv_min, rvv_max)
    ...
    new_rlist = np.concatenate(([1 for x in range(len(previous_positions))], mean_rlist))
    new_types = np.concatenate((previous_types, [maxcomp for x in range(len(mean_positions))]))
    new_clusters = np.concatenate(([0 for x in range(len(previous_positions))], mean_cluster))
    atom_dict = {"positions": new_positions, 
                 "types": new_types, 
                 "rvv": new_rlist,
