...
def test_calculate_elastic_constants_emt_cu(tmp_path):
    ...
    assert (
        constants["mechanically_stable"] is True
    ), f"C eigenvalues {constants['stiffness_eigenvalues']}"
    assert C[0, 0] > 0 and C[0, 0] > C[0, 1]
    assert C[3, 3] > 0
    assert constants["K_VRH"] > 0 and constants["G_VRH"] > 0
    # EMT Cu bulk modulus is ~ 130-180 GPa range
    assert 80 < constants["K_VRH"] < 250
