...
def Create_minimal_cell_Method_1(sigma, uvw, R):
    ...
    indice_0 = indice[np.where(np.sum(abs(indice), axis=1) != 0)]
    condition1 = (abs(dot(indice_0, uvw) / norm(indice_0, axis=1) / norm(uvw))).round(7)
        ...
        if 1 - ang(TestVecs[count], MiniCell_1[:, 2]) > tol:
            ...
            for j in range(len(TestVecs)):
                if (1 - ang(TestVecs[j], MiniCell_1[:, 2]) > tol) and (
                    ...
                    if (
                        ang(TestVecs[j], cross(MiniCell_1[:, 2], MiniCell_1[:, 1]))
                        > tol
                    ):
                        ...
                        MiniCell_2 = (np.round(dot(R, MiniCell_1), 7)).astype(int)
                        Det2 = abs(round(det(MiniCell_2), 5))

                        if (abs(Det1 - sigma)) < tol and (abs(Det2 - sigma)) < tol:
                            Found = True
                            break
    if Found:
...
def Basis(basis):
    ...
    if str(basis) == "fcc":
        basis = np.array(
            [[0, 0, 0], [0.5, 0, 0.5], [0.5, 0.5, 0], [0, 0.5, 0.5]], dtype=float
        )
    elif str(basis) == "bcc":
...
def Find_Orthogonal_cell(basis, uvw, m, n, GB1):
    ...
    if CommonDivisor(OrthoCell_1[:, 2])[1] == CommonDivisor(OrthoCell_2[:, 2])[1]:
...
def odd_even(M1):
    ...
    """
    d_e = np.array([["a", "a", "a"], ["a", "a", "a"], ["a", "a", "a"]], dtype=str)
    for i in range(3):
        for j in range(3):
            if abs(M1[i][j]) % 2 == 0:
                d_e[i][j] = "e"
...
def face_centering(a):
    ...
    for i in range(3):
        if (
            np.all(odd_even(M)[:, i] == ["d", "d", "e"])
            or np.all(odd_even(M)[:, i] == ["e", "d", "d"])
            or np.all(odd_even(M)[:, i] == ["d", "e", "d"])
        ):
            count = count + 1
            z_f[i][i] = 0.5
    if det(z_f) == 0.25:
        ...
        for i in [0, 1]:
            for j in [1, 2]:
                if i != j and count < 2:
                    ...
                    M[:, j] = M[:, i] + M[:, j]
                    if (
                        np.all(odd_even(M)[:, j] == ["d", "d", "e"])
                        or np.all(odd_even(M)[:, j] == ["e", "d", "d"])
                        or np.all(odd_even(M)[:, j] == ["d", "e", "d"])
                    ):
                        count = count + 1
                        z_f[i][j] = 0.5
                        z_f[j][j] = 0.5
...
def Ortho_fcc_bcc(basis, O1, O2):
    ...
    if basis == "fcc":
        ...
        base = (np.array([[0.5, 0.5, 0.5], [0.5, 0.5, -0.5], [-0.5, 0.5, 0.5]])).T

    for i in range(3):
        Min_d = min(
            CommonDivisor(dot(O1[:, i], inv(base)))[1],
...
def Write_to_io(axis, m, n, basis):
    ...
    my_dict = {
        "GB_plane": str([axis[0], axis[1], axis[2]]),
        "lattice_parameter": "4",
    ...
    with open("io_file", "w") as f:
        ...
        f.write(
            "# CSL plane of interest that you read from the output of "
        ...
        f.write(
            "# atoms that are closer than this fraction of the lattice "
            "parameter will be removed \n"
        )
        f.write(
            "# either from grain1 (g1) or from grain2 (g2). If you choose "
        ...
        f.write(
            list(my_dict.keys())[3] + ": " + str(list(my_dict.values())[3]) + "\n\n"
        )
        f.write(
            "# decide whether you want rigid body translations to be done "
        ...
        f.write(list(my_dict.keys())[4] + ": " + str(list(my_dict.values())[4]) + "\n")

        f.write(list(my_dict.keys())[5] + ": " + str(list(my_dict.values())[5]) + "\n")

        f.write(
            list(my_dict.keys())[6] + ": " + str(list(my_dict.values())[6]) + "\n\n"
        )

        f.write(
            "# dimensions of the supercell in: [l1,l2,l3],  where l1 is"
        ...
        f.write(
            "# The following is your csl_generator output."
