...
class TestBulkFunctions(unittest.TestCase):
    ...
    def test_generate_structures_iso(self):
        ...
        for struct, eps in zip(structures, expected_strains):
            ...
            np.testing.assert_allclose(
                np.array(struct.get_cell()), scaled, rtol=1e-7, atol=1e-12
            )
    ...
    def test_generate_structures_axis_a(self):
        ...
        for i, struct in enumerate(structures):
            ...
            self.assertNotEqual(
                np.linalg.norm(new_cell[0]), np.linalg.norm(original_cell[0])
            )
    ...
    def test_generate_structures_axis_b(self):
        ...
        for struct in structures:
            ...
            self.assertNotEqual(
                np.linalg.norm(new_cell[1]), np.linalg.norm(original_cell[1])
            )
    ...
    def test_generate_structures_axis_c(self):
        ...
        for struct in structures:
            ...
            self.assertNotEqual(
                np.linalg.norm(new_cell[2]), np.linalg.norm(original_cell[2])
            )
